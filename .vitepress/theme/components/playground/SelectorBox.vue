...
import { ref, triggerRef, computed, defineProps, watch, defineEmit } from 'vue'
...
const directions = ref(props.modelValue || [])
...
watch(
  () => props.modelValue,
  v => directions.value = v || [],
)
</script>

<template>
  <div class="bg-gray-100 w-22 h-22 rounded-2xl flex">
    <div class="w-20 h-20 relative m-auto">
      <!-- Corners -->
      <template
      ...
      <div
        class="
          ...
          border-gray-300 left-1/2 top-1/2 -translate-y-1/2 -translate-x-1/2
        "
        @click="toggleAll()"
      />
    </div>
...
.item.has:not(.active) {
  @apply border-light-blue-400 border-opacity-50;
}
</style>
